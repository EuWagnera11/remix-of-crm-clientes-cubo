      ...
      <Card>
        ...
        <CardContent>
          {consents.length === 0 ? (
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Paciente</TableHead><TableHead>Tipo</TableHead><TableHead>Status</TableHead><TableHead>Data</TableHead></TableRow></TableHeader>
    ...
    <>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><Label>Profissional (opcional)</Label><Input value={goalForm.professional_name} onChange={e => setGoalForm(f => ({ ...f, professional_name: e.target.value }))} placeholder="Geral da clínica" /></div>
          </div>
      ...
      {goals.length > 0 && (
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader><TableRow><TableHead>Período</TableHead><TableHead>Meta</TableHead><TableHead>Profissional</TableHead><TableHead className="w-[80px]">Ações</TableHead></TableRow></TableHeader>
...
function PostProcedureTab({ clinicId }: { clinicId: string }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Template pós-procedimento salvo!" });
    },
    onError: (e: any) => toast({ title: "Erro", description: e.message, variant: "destructive" }),
  });
  ...
  return (
    <>
      <Card>
        <CardHeader><CardTitle className="text-sm">Novo Template Pós-Procedimento</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><Label>Nome do fluxo *</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ex: Pós-clareamento" /></div>
            <div><Label>Procedimento (opcional)</Label>
          ...
          <div><Label>Mensagem (use {`{{nome}}`} para o nome do paciente)</Label>
            <Textarea value={form.message_template} onChange={e => setForm(f => ({ ...f, message_template: e.target.value }))} placeholder="Olá {{nome}}, como você está se sentindo após o procedimento?" className="min-h-[100px]" />
          </div>
...
export default function SettingsPage() {
  ...
  const createMemberMutation = useMutation({
    mutationFn: async () => { if (!effectiveClinicId) throw new Error("Nenhuma clínica"); const { data, error } = await supabase.functions.invoke("manage-team", { body: { action: "create", ...newMember, clinic_id: effectiveClinicId } }); if (error) throw error; if (data?.error) throw new Error(data.error); },
  ...
  const saveMutation = useMutation({
    mutationFn: async () => { if (!effectiveClinicId) throw new Error("Nenhuma clínica"); const { error } = await supabase.from("clinics").update({ name: clinicForm.name, phone: clinicForm.phone || null, email: clinicForm.email || null }).eq("id", effectiveClinicId); if (error) throw error; },
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="whitelabel">
        <TabsList className="bg-accent flex-wrap h-auto gap-1">
          ...
          <TabsTrigger value="post-procedure">Pós-Procedimento</TabsTrigger>
        ...
        <TabsContent value="clinic" className="mt-4 space-y-4">
          ...
          <Card className="bg-card">
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div><Label>Nome da Clínica</Label><Input value={clinicForm.name} onChange={e => setClinicForm(f => ({ ...f, name: e.target.value }))} /></div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div><Label>Cargo</Label>
                        <Select value={newMember.role} onValueChange={v => setNewMember(m => ({ ...m, role: v }))}>
                          ...
                          <SelectContent><SelectItem value="clinic_owner">Proprietário</SelectItem><SelectItem value="clinic_staff">Profissional</SelectItem><SelectItem value="clinic_receptionist">Recepcionista</SelectItem></SelectContent>
                        </Select>
                      </div>
